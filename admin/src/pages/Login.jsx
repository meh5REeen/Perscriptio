  ...
  return (
    <form  onSubmit = {onSubmitHandler} className='min-h-[80vh] flex items-center'>
      ...
      <div className='w-full'>
        ...
        onClick={(e)=>setEmail(e.target.value)} value={email}
      ...
      <div className='w-full'>
        ...
        onChange={(e)=>setPassword(e.target.value)}
         onClick={(e)=>setPassword(e.target.value)} value={password}
        className='border border-[#DADADA] rounded w-full p-2 mt-1' type="password" required/>
      </div>
      <button className='bg-[#5f6FFF] text-white w-full py-2 rounded-md text-base' type="submit">Login</button>
      {
        state === 'Admin' ? <p>Doctor Login? <span className='text-[#5f6FFF] cursor pointer' onClick={()=>setState('Doctor')}>Click Here</span></p>:
