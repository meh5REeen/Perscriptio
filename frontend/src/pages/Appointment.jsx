...
const Appointment = () => {
  ...
  const bookAppointment = async ()=>{
    if(!token){
      ...
      setTimeout(() => navigate('/login'), 1500); // wait 1.5s before redirect
      return;
    }
    try{
  ...
  return docInfo && (
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-[#5f6FFF] w-full sm:max-w-72 rounded-lg'
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='text-gray-500 font-medium mt-4'>
            Appointment free: <span className='text-gray-600'>
        ...
        {docSlots.length && docSlots[slotIndex].map((item ,index) => (
          <p className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
           ...
           onClick={() => {
                  ...
                  setSlotTime(item.time); // keep track of selected time for booking
                }}
           >
